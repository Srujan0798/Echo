...
const FullProfileModal: React.FC<FullProfileModalProps> = ({ profile, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center animate-fade-in" onClick={onClose}>
      <div 
        className="bg-[#1a1a1a] w-full max-w-md max-h-[85vh] rounded-t-2xl flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className={`relative w-full h-40 rounded-t-2xl ${profile.vibeGraphic} flex flex-col justify-end p-4 shadow-lg`}>
          <button onClick={onClose} className="absolute top-4 right-4 text-white bg-black/40 rounded-full p-2 z-10"><X size={24} /></button>
        ...
        <div className="p-4 space-y-4 overflow-y-auto">
          ...
          <div>
            ...
            <div className="space-y-3">
              <PromptPlayer prompt="If you were a ghost, who would you haunt?" />
              <PromptPlayer prompt="What's the most useless talent you have?" />
              <PromptPlayer prompt="Describe your perfect day with only sound effects." />
            </div>
          </div>
        </div>
      </div>
